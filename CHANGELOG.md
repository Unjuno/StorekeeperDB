...
- Public alpha docs: demo, React verification, magic lifecycle, transaction model, browser boundary, audit notes, next-work plan, and todo example.
...
- Derived projections can be marked cold, garbage-collected, budget-limited, and rebuilt from source rows.
...
- CI now runs `npm run release:check`, including export artifact checks, React verification, the executable demo, lifecycle tests, and `npm pack --dry-run`.
...
- Automatic time-based lifecycle decay is not re-imported yet.
- API is not frozen.
